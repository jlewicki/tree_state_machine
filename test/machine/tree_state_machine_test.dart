  ...
  group('TreeStateMachine', () {
    ...
    group('saveTo', () {
      ...
      test('should write state data of active states to sink ', () async {
        ...
        final encodableTree = EncodableTree.fromJson(jsonList[0] as Map<String, dynamic>);
    ...
    group('loadFrom', () {
      ...
      test('should read active data states from stream ', () async {
        ...
        var currentState = await sm.start(tree.r_a_a_1_key);
        ...
        await sm.loadFrom(Stream.fromIterable(encoded));
        expect(sm.isStarted, isTrue);
        expect(currentState.key, tree.r_a_a_1_key);
      ...
      test('should throw if stream does not contain Map<string, dynamic>', () async {
        var sm = TreeStateMachine(tree.treeBuilder());
        var byteStream =
            Stream.fromIterable(<Object?>['A', 'B']).transform(json.fuse(utf8).encoder);
    ...
    group('dataStream', () {
      test('should notify listeners if subscribed before states are active', () async {
        final sm = TreeStateMachine(data_tree.treeBuilder(
          initialDataValues: {
          ...
          messageHandlers: {
            data_tree.r_a_a_2_key: (msgCtx) {
              ...
              msgCtx.data<LeafDataBase>()!.update((current) => current..name = 'you');
      ...
      test('should notify listeners if subscribed while states are active', () async {
        final sm = TreeStateMachine(data_tree.treeBuilder(
          initialDataValues: {
            data_tree.r_a_a_key: () => LeafDataBase()..name = 'me',
            data_tree.r_a_a_2_key: () => LeafData2()..label = 'cool'
      ...
      test('should notify listeners if subscribed after states are active', () async {
        final sm = TreeStateMachine(data_tree.treeBuilder(
          initialDataValues: {
            data_tree.r_a_a_key: () => LeafDataBase()..name = 'me',
            data_tree.r_a_a_2_key: () => LeafData2()..label = 'cool'
