...
///
/// A [TreeStateMachine] is constructed with a [StateTreeBuildProvider] that
/// defines the specific tree of states that the state machine manages. After
/// the state machine is constructed, calling [start] will enter the initial
/// state for the tree, and return a [CurrentState] that serves as a proxy for
/// the current state of the state tree. [CurrentState.post] can be used to send
/// a message to the state for processing, which may result in a transition to a
/// new state.
///
/// ```dart
...
///   print('The current state after processing a '
///         'message is ${currentState.key}');
/// ```
///
/// When the machine is started, the machine determines determine a path of
/// states, starting at the root and ending at a leaf state, by recursively
/// determining the `initialChild` at each level of the tree until a state with
/// no children is reached. This path of states is called the initial path, and
/// the machine will call `onEnter` for each state along this path. When all the
/// state along the path have been entered, the state at the end of the path
/// becomes the current state of the state machine.
///
/// ## Event Streams
///
/// [TreeStateMachine] provides several event streams that may be used to
/// observe how messages sent to the machine are processed, and what state
/// transitions occur.
///  * [processedMessages] yields an event for every message that is processed
///    by the tree, whether or not the message was handled successfully, and
///    whether or not a transition occurred as a result of the message.
///  * [handledMessages] is a convenience stream that yield an event when only
///    when a message is successfully handled.
///  * [transitions] yields an event each time a transition between states
///    occurs.
///
/// ## Error Handling
///
/// Errors may occur when as state processes a message. This can happen in the
/// [TreeState.onMessage] handler while the state is processing the message, or
/// it can happen during a state transition in the [TreeState.onExit] handler of
/// one of the states that is being exited, or in the [TreeState.onEnter]
/// handler of one of the states that is being entered.
///
/// In either case the state machine catches the error internally, converts it
/// to a [FailedMessage], and yields it from he future returned from
/// [CurrentState.post]. The [FailedMessage] is also emitted on the
...
class TreeStateMachine {
  ...
  /// This factory may be useful when a custom [TreeBuildContext] is used during
  /// tree construction, but the [TreeStateMachine.new] factory is more commonly
  /// used.
  ///
  ...
  ///
  /// Returns `null` if the state machine has not been started, or if it is
  /// disposed.
  CurrentState? get currentState => _currentState;

  /// Returns `true` if the state machine has ended.
  ///
  /// A state machine is done when a final state is entered. This may have
  /// occurred because transition to a final state has occurred as result of
  /// processing a message, or because [stop] was called.
  ...
  ///
  /// An event is emitted on this stream as the state machine moves through its
  /// lifecycle. For example, [LifecycleState.started] will be emitted when
  /// [start] is called, and the returned future completes.
  ValueStream<LifecycleState> get lifecycle => _lifecycle.states;

  /// A broadcast stream of [Transition] events.
  ///
  /// A [Transition] is emitted on this stream when a state transition occurs
  /// within the state machine.
  Stream<Transition> get transitions => _transitions.stream;

  /// A broadcast stream of [ProcessedMessage] events.
  ///
  /// A [ProcessedMessage] event is raised on this stream when a message was
  /// processed by a state within the state machine. The result of this
  /// processing may have resulted in a state transition, in which case an event
  /// will also be raised on the [transitions] stream.  When this occurs, an
  /// event on this stream is raised first.
  ///
  /// Note that the [ProcessedMessage] event does not necessarily mean that the
  /// message was handled successfully; it might have been unhandled or an error
  /// might have occurred. Check the runtime type of the event to determine what
  /// occurred.
  Stream<ProcessedMessage> get processedMessages => _processedMessages.stream;

  /// A broadcast stream of [HandledMessage] events.
  ///
  /// A [HandledMessage] is raised on this stream when a message was
  /// successfully handled by a state within the state machine.
  ///
  /// Note that the [HandledMessage] is also raised on the [processedMessages]
  /// stream.
  ...
  ///
  /// A [FailedMessage] is raised on this stream when an error was thrown from
  /// one of a states handler functions while a message was being handled or
  /// during a state transition.
  ///
  /// Note that the [FailedMessage] is also raised on the [processedMessages]
  /// stream.
  ...
  ///
  /// Each node in the state tree is accessible from this node and its
  ...
  /// Starts the state machine, transitioning the current state to the initial
  /// state of the state tree. Returns a [CurrentState] that can be used to send
  /// messages for processing.
  ///
  /// [at] may be used to indicate the initial state. If provided, the state
  /// machine will transition from the root state to this state. If the initial
  /// state is a leaf state, that will be the current state when the returned
  /// future completes. Otherwise, the state machine will follow the initial
  /// child path for the initial state, until a leaf state is reached. This leaf
  ...
  /// data state that is entered. If it does not contain a value for a data
  /// state, the [InitialData] associated with that data state will used
  /// instead.
  ///
  /// Note that while [withData] can be used to start or restore the state
  /// machine with a specific set of values, it is of course possible that these
  /// values may be ones that would never be produced by the state machine
  /// itself when started without [withData], potentially breaking invariants
  /// expected by the state tree. As a result, [withData] is intended primarily
  /// for development and testing purposes, and care should be taken when using
  /// it.
  ///
  /// It is safe to call [start] when the state machine is already started. It
  /// is also safe to call [start] if the state machine has been stopped, in
  /// which case the state machine will be restarted, and will re-enter the
  /// initial state.
  ...
  ///
  /// Stopping the state machine will cause a transition to a final state
  /// identified by [stoppedStateKey]. This transition is irrevocable, and the
  /// message handler of the current leaf state will not be called before the
  /// transition occurs.
  ///
  /// When the returned future completes, the the [CurrentState.key] will be
  /// [stoppedStateKey], and [isDone] will return true.
  ///
  /// Because the state machine could potentially be restarted, stopping the
  /// state machine does not complete state machine streams such as
  /// [transitions]. If the state machine will never be restarted, [dispose] can
  /// be used to complete the streams, or any subscriptions may explicitly be
  /// canceled.
  ///
  ...
  ///
  /// Disposing the state machine completes the all of its streams, and will
  /// permanently mark it as disposed and unusable. Any future calls to methods
  /// that update the state machine will throw a [DisposedError].
  ///
  /// Additionally, the [DataValue] in each data state will be marked as
  /// complete. Note however that listeners to the [DataValue] will not be
  /// notified of completion until the next microtask.
  ///
  ...
  void dispose() {
    _lifecycle.dispose(() {
      ...
      // We just disposed the nodes, which means the data values for any data
      // states were closed. However the Done notifications for those data
      // values are sent asynchronously in a microtask, so we cant close data
      // streams (which are listening to the data value streams) until those
      // Done notifications are flushed. So use a timer to wait for this (since
      // timers run on the event loop, which is processed at lower priority than
      // microtasks).
  ...
  ///
  /// A data tree state has an associated data value of type [D]. As messages
  /// are processed by the state, it may update its data value. Each time the
  /// value changes, the new value is published to this stream.
  ///
  ...
  ///
  /// Saving the active state data allows the state of running state machine to
  /// be written to external storage. This state can be reloaded at a later
  /// time, potentially in a seperate application session, into the state
  /// machine using [loadFrom].
  ///
  /// Only information about the active states (the current leaf state and its
  /// ancestors) is saved. This means that when restoring, the state machine on
  /// which [loadFrom] is called must be constructed with the same tree
  /// definition as this state machine.
  ///
  /// The state machine must be started before this method is called, otherwise
  ...
  /// Initializes the active state data of the state machine by reading the
  /// specified stream.
  ///
  /// The data contained in [stream] should have been generated by a previous
  /// call to [saveTo] on a state machine that is was constructed with the same
  /// tree definition as this machine.
  ///
  /// Note that this method can only be called on a state machine that has not
  /// been started. When the returned future completes, the state machine will
  /// have been started, with the current state matching the current state
  ...
  Future<CurrentState> loadFrom(Stream<List<int>> stream) async {
    ...
    if (objectList[0] is! Map<String, dynamic>) {
      throw ArgumentError.value(
        stream,
        'stream',
        'Found ${objectList[0].runtimeType} in stream. '
    ...
    final mismatchedActivePath = StateError(
        'Active path in stream [$encodedActivePath] does not match active path '
        'in state machine [$treeActivePath]');
    ...
    // Start state machine so that the active nodes matches that in the encoded
    // data.
    return start(
      ...
      withData: (b) {
        for (var i = 0; i < activeNodes.length; ++i) {
          final es = encodableTree.states[i];
          final node = activeNodes[i];
          // It's not useful to have a DataTreeState<void>, but it is not
          // prohibited, so skip those states (there is no data to set)
          if (node.state is DataTreeState &&
              node.state is! DataTreeState<void>) {
            if (node.info.dataCodec == null) {
              throw StateError(
                  'Unable to deserialize state data because a serializer has '
                  'not been specified for state ${node.key}');
            }
...
enum PostMessageErrorPolicy {
  /// The error is caught and converted to a [FailedMessage] that is returned
  /// when the future returned by [CurrentState.post] is awaited.
  ///
  /// Note that this conversion does *not* apply if the error is
  /// [StateMachineError]. Errors of that type will always be rethrown.
  convertToFailedMessage,

  /// The error is rethrown when the future returned by [CurrentState.post] is
  /// awaited.
  rethrowError,
}
...
///
/// [CurrentState] provides information about the current leaf state and its
/// ancestor states, as well as the data value of any active data states. These
/// values change over time as messages are processed and state transitions
/// occur.
///
/// Messages can be sent to the leaf state for processing using the [post]
/// method.
class CurrentState {
  ...
  /// Returns the value stream of state data type [D] associated with an active
  /// data state.
  ///
  /// Starting with the current leaf state, each active state is visited. If a
  /// state has a state data value that matches [D], then the value stream for
  /// that state is returned. If [key] is provided, then the value is only
  /// returned if the key matches an active state.
  ///
  /// If [D] is `dynamic`, then the data for the current leaf state is returned,
  /// or `null` if the current leaf state is not a data state.
  ///
  /// If [D] is `void`, `null` is returned.
  ///
  /// The retured stream completes when the state to which it corresponds is no
  /// longer active. If a long lived stream is needed that remains valid as the
  /// state becomes inactive then active again, then
  /// [TreeStateMachine.dataStream] can be used.
  ValueStream<D>? dataStream<D>(DataStateKey<D> key) {
  ...
  /// Returns `true` if the specified state is an active state in the state
  /// machine.
  ///
  /// The current leaf state, and all of its ancestor states, are considered
  /// active states.
  ...
  /// The [StateKey]s identifying the states that are currently active in the
  /// state machine.
  ///
  /// The current leaf state is first in the list, followed by its ancestor
  /// states, and ending at the root state.
  ...
  ///
  /// Messages are buffered and processed in-order, so it is safe to call [post]
  /// multiple times without waiting for the futures returned by earlier calls
  /// to complete.
  ///
  /// Returns a future that yields a [ProcessedMessage] describing how the
  /// message was processed, and any state transition that occured.
  ///
  /// If an error occurred while processing the message, and the state machine
  /// was created with [PostMessageErrorPolicy.rethrowError], then the future
  /// will throw the error when awaited. Otherwise, the future will yield a
  /// [FailedMessage] when awaited.
  Future<ProcessedMessage> post(Object message) {
    return stateMachine._queueMessage(message);
  }
}

// Helper class pairing a message, and the completer that will signal the
